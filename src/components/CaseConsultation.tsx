...
export default function CaseConsultation() {
  ...
  return (
    <section id="consultar" className="py-24 bg-stone-950">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-serif mb-4 text-stone-50">Consulta de Expedientes</h2>
        ...
        <div className="bg-stone-900 p-8 rounded-2xl border border-stone-800 shadow-sm mb-12">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-12 pr-12 py-4 bg-stone-800 border border-stone-700 text-stone-100 rounded-xl focus:ring-2 focus:ring-stone-100 focus:border-transparent outline-none transition-all placeholder:text-stone-600"
                required
              />
                ...
                <AnimatePresence>
                  {showHelp && (
                    <motion.div
                      ...
                      className="absolute bottom-full right-0 mb-4 w-72 bg-stone-800 border border-stone-700 p-4 rounded-xl shadow-2xl z-50 text-left"
                    >
                      <div className="space-y-3">
                        <div>
                          <p className="text-stone-100 text-xs font-bold uppercase tracking-wider mb-1">NUC</p>
                          <p className="text-stone-400 text-[11px] leading-relaxed">
                            Número Único de Caso: Identificador oficial de expedientes judiciales para seguimiento de todo el proceso.
                          </p>
                        </div>
                        <div>
                          <p className="text-stone-100 text-xs font-bold uppercase tracking-wider mb-1">Solicitud</p>
                          <p className="text-stone-400 text-[11px] leading-relaxed">
                      ...
                      <div className="absolute bottom-0 right-6 translate-y-1/2 rotate-45 w-3 h-3 bg-stone-800 border-r border-b border-stone-700" />
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-stone-100 text-stone-900 px-8 py-4 rounded-xl font-medium hover:bg-white transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
        ...
        <AnimatePresence mode="wait">
          {error && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="p-6 bg-red-950/20 border border-red-900/50 rounded-2xl flex items-start gap-4"
            >
              <AlertCircle className="text-red-500 w-6 h-6 shrink-0" />
              <div>
                <h4 className="font-bold text-red-200">Error en la consulta</h4>
                <p className="text-red-400 text-sm">{error}</p>
                <button 
                  onClick={clearSearch}
                  className="mt-2 text-xs text-red-500 underline font-medium"
                >
                  Limpiar búsqueda
                </button>
              </div>
          ...
          {result && (
            <motion.div
              ...
              className="space-y-6"
            >
              <div className="bg-stone-900 border border-stone-800 rounded-2xl p-8 shadow-sm">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                  <div>
                    ...
                    <h3 className="text-2xl font-serif text-stone-50">{result.title}</h3>
                    <p className="text-stone-500 text-sm mt-1">Cliente/Partes: {result.client}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-stone-500 uppercase tracking-wider font-bold">Última actualización</p>
                    <p className="text-stone-300 font-medium">{result.lastUpdate}</p>
                  </div>
                </div>

                <div className="mb-8">
                  <h4 className="text-sm font-bold text-stone-500 uppercase tracking-wider mb-3">Descripción</h4>
                  <p className="text-stone-400 leading-relaxed">{result.description}</p>
                </div>

                {result.steps && result.steps.length > 0 && (
                  <div>
                    <h4 className="text-sm font-bold text-stone-500 uppercase tracking-wider mb-6">Línea de Tiempo del Proceso</h4>
                    <div className="space-y-8 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-px before:bg-stone-800">
                      {result.steps.map((step, idx) => (
                        <div key={idx} className="relative pl-10">
                          <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-2 flex items-center justify-center bg-stone-900 z-10 ${
                            step.completed ? 'border-emerald-500 bg-emerald-950/30' : 'border-stone-800'
                          }`}>
                            {step.completed ? (
                          ...
                          <div className="flex flex-col md:flex-row md:items-center justify-between gap-1">
                            <span className={`font-medium ${step.completed ? 'text-stone-100' : 'text-stone-600'}`}>
                              {step.title}
                            </span>
                            {step.date && (
              ...
              {(() => {
                ...
                return (
                  <motion.div 
                    ...
                    className="bg-stone-100 rounded-2xl p-8 text-stone-900 shadow-xl overflow-hidden relative"
                  >
                    <div className="absolute top-0 right-0 w-64 h-64 bg-stone-900/5 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl" />
                    
                    <div className="relative z-10">
                      <div className="flex items-center gap-2 text-stone-500 text-xs font-bold uppercase tracking-widest mb-6">
                        <User className="w-4 h-4" />
                        Especialista Recomendado
                      </div>
                      
                      <div className="flex flex-col md:flex-row items-center gap-8">
                        <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-stone-900/10 shrink-0">
                          <img 
                            src={lawyer.image} 
                            alt={lawyer.name} 
                        ...
                        <div className="flex-1 text-center md:text-left">
                          ...
                          <p className="text-stone-600 text-sm mb-4">{lawyer.specialty}</p>
                          <p className="text-stone-700 text-sm leading-relaxed mb-6 italic">
                            "{lawyer.bio}"
                          </p>
                          <button className="inline-flex items-center gap-2 text-stone-900 font-bold text-sm hover:gap-3 transition-all">
              ...
              <div className="flex justify-center gap-4">
                <button 
                  onClick={clearSearch}
                  className="text-stone-500 hover:text-stone-100 text-sm font-medium transition-colors"
                >
                  Nueva consulta
                </button>
                <button className="text-stone-500 hover:text-stone-100 text-sm font-medium flex items-center gap-1 transition-colors">
